    ...
    async function fetchDailyTotals() {
        try {
            ...
            if (result.status === 'success') {
                ...
                // Update the TDEE display
                document.getElementById('tdee-display').textContent = `Your TDEE is ${data.tdee} calories`;
                
                // Update each macro progress bar and percentage
                updateProgress('calories', data.consumed.calories, data.tdee);
                updateProgress('protein', data.consumed.proteins, data.protein_goal);
                updateProgress('carb', data.consumed.carbs, data.carbs_goal);
                updateProgress('fat', data.consumed.fats || 0, data.fat_goal); 
            } else {
